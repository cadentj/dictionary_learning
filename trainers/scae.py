...
class SCAESuite(nn.Module):
    ...
    def get_initial_contribs_attn(
        ...
        initial_act = cache["blocks.0.hook_resid_pre"]  # [batch, seq, d_model]
        initial_act_post_ln = (
            initial_act / cache[f"blocks.{layer}.ln1.hook_scale"]
        )
    ...
    def get_pruned_contribs_attn(
        ...
        if self.connections is None:
            ...
            up_recons_post_ln = (
                up_recons / cache[f"blocks.{layer}.ln1.hook_scale"]
            )

            post_ov = einsum(
                up_recons_post_ln,
                W_OV,
                "b s d_model, n_heads d_model d_out -> b s n_heads d_out",
            )
            probs = cache[f"blocks.{layer}.attn.hook_pattern"]
            post_moving = einsum(
                post_ov, probs, "b k h d_out, b h q k -> b q d_out"
            )
            ...
            # Compute temp tensor
            temp = einsum(
                self.aes[down_name].encoder.weight,
                W_OV,
            ...
            selected_up = self.aes[up_name].decoder.weight[
                :, j_indices
            ]  # [d_in, M]
            values = einsum(
                selected_temp,
                selected_up,
                "n_heads m d_in, d_in m -> n_heads m",
            )
            ...
            # Divide by layer norm scale
            up_facts_selected = (
                up_facts_selected / cache[f"blocks.{layer}.ln1.hook_scale"]
            )

            # Expand dimensions for vectorized scatter
            scaled = (
                up_facts_selected[:, :, None, :] * values[None, None, :, :]
            )  # [B, S, n_heads, M]
            ...
            probs = cache[f"blocks.{layer}.attn.hook_pattern"]
            final_contribs = einsum(
                probs, contributions, "b h q k, b k h f -> b q f"
            )
    ...
    def forward_pruned(
        ...
        for down_name in self.submodule_names:
            ...
            for up_name in self.submodule_names:
                ...
                if up_layer > down_layer or (
                    up_layer == down_layer
                    and (up_type == "mlp" or up_name == down_name)
                ):
                    continue
                ...
                if self.connections is None or (
                    ...
                    if down_type == "attn":
                        contributions = self.get_pruned_contribs_attn(
                            cache, up_name, down_name, up_feats
                        )
                    else:
            ...
            if down_type == "attn":
                approx_acts = (
                    approx_acts
                    + self.model.b_O[down_layer].squeeze().to(self.dtype)
                    @ self.aes[down_name].encoder.weight.T
                )

            # Add downstream b_enc
            bias = self.aes[down_name].encoder.bias
            bias = (
                bias.unsqueeze(0) if bias.dim() < approx_acts.dim() else bias
            )
            ...
            upstream_bias_post_ln = (
                ...
                / cache[f"blocks.{down_layer}.{ln_name}.hook_scale"]
            )
            if down_type == "attn":
                upstream_bias_post_ln = einsum(
                    self.W_OVs[down_layer],
                    upstream_bias_post_ln,
            ...
            reconstructions[down_name] = self.aes[down_name].decode(
                feat_buffer
            )
